            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              {messages.length === 0 && (
                <div className="rounded-xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                  Try “How does my agent experience connect across internships and projects?” or “What evidence supports my ML systems background?”
                </div>
              )}
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  {result.retrieval.queries.map((query) => (
                    <article key={query.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                      ...
                      <p className="mt-2 text-xs text-slate-400">{query.hitIds.length} semantic matches</p>
                    </article>
                  ))}
                </div>
              </section>
